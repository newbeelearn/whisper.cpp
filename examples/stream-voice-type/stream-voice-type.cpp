...
#if defined(__linux__) && __has_include(<X11/Xlib.h>) && __has_include(<X11/keysym.h>)
#define WHISPER_STREAM_VT_X11_HOTKEY 1
#include <X11/Xlib.h>
...
static bool whisper_params_parse(int argc, char ** argv, whisper_params & params) {
    for (int i = 1; i < argc; i++) {
        ...
        else if (arg == "-gks"  || arg == "--global-key-start-stop") { params.hotkey_toggle = true; }
...
void whisper_print_usage(int /*argc*/, char ** argv, const whisper_params & params) {
    ...
    fprintf(stderr, "  -gks,     --global-key-start-stop [%-7s] toggle stream start/stop with Ctrl+Alt+S\n", params.hotkey_toggle ? "true" : "false");
    fprintf(stderr, "\n");
}


class global_hotkey_toggle {
public:
    explicit global_hotkey_toggle(bool enabled) : m_enabled(enabled) {}
    ...
    bool start() {
        ...
        m_running = true;
        m_thread = std::thread(&global_hotkey_toggle::thread_loop, this);
#else
        fprintf(stderr, "%s: global hotkey requires Linux/X11 headers; running without hotkey support\n", __func__);
#endif
        return true;
    }
...
private:
    ...
    void thread_loop() {
        ...
        if (display == nullptr) {
            fprintf(stderr, "%s: failed to open X11 display, global hotkey disabled\n", __func__);
            return;
        }
